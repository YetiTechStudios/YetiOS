...
struct FYetiOsMotherBoard : public FYetiOsHardware
{
	...
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Yeti OS Mother Board")
	EYetiOsSocketType MotherboardSocketType;

	/* Current durability of this board. Lower durability tends to break faster. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Yeti OS Mother Board", meta = (UIMin = "0", ClampMin = "0", UIMax = "1", ClampMax = "1"))
	float MotherboardDurability;

	/* Price of this board. */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, Category = "Yeti OS Mother Board", meta = (UIMin = "0", ClampMin = "0", UIMax = "4000", ClampMax = "4000"))
	...
	FORCEINLINE const bool AllCpusAreOfCorrectSocketType(FYetiOsCpu& IncorrectCpu) const
	{
		for (int32 i = 0; i < 4; ++i)
		{
			if (MotherboardCpus.IsValidIndex(i))
