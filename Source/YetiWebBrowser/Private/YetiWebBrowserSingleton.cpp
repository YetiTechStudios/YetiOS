...
#include "IYetiWebBrowserCookieManager.h"


DEFINE_LOG_CATEGORY_STATIC(LogYetiWebBrowser, Log, All);
...
#if WITH_CEF3
#	define CEF3_BIN_DIR TEXT("YetiOS/Binaries/ThirdParty/CEF")
...
FString FYetiWebBrowserSingleton::ApplicationCacheDir() const
{
#if PLATFORM_MAC
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
class FYetiWebBrowserWindowFactory
	: public IYetiWebBrowserWindowFactory
{
public:

	virtual ~FYetiWebBrowserWindowFactory()
	{ }

	virtual TSharedPtr<IYetiWebBrowserWindow> Create(
		TSharedPtr<FCEFWebBrowserWindow>& BrowserWindowParent,
		TSharedPtr<FWebBrowserWindowInfo>& BrowserWindowInfo) override
	{
		return IYetiWebBrowserModule::Get().GetSingleton()->CreateBrowserWindow(
			BrowserWindowParent,
			BrowserWindowInfo);
	}
...
class FYetiNoWebBrowserWindowFactory
	: public IYetiWebBrowserWindowFactory
{
public:

	virtual ~FYetiNoWebBrowserWindowFactory()
	{ }
...
FYetiWebBrowserSingleton::FYetiWebBrowserSingleton(const FYetiWebBrowserInitSettings& WebBrowserInitSettings)
#if WITH_CEF3
	: WebBrowserWindowFactory(MakeShareable(new FYetiWebBrowserWindowFactory()))
#else
	: WebBrowserWindowFactory(MakeShareable(new FYetiNoWebBrowserWindowFactory()))
#endif
...
{
#if WITH_CEF3
	// The FYetiWebBrowserSingleton must be initialized on the game thread
	...
	CEFBrowserApp->OnRenderProcessThreadCreated().BindRaw(this, &FYetiWebBrowserSingleton::HandleRenderProcessCreated);
	...
	FString CefLogFile(FPaths::Combine(*FPaths::ProjectLogDir(), TEXT("cef.log")));
	CefLogFile = FPaths::ConvertRelativePathToFull(CefLogFile);
	CefString(&Settings.log_file) = TCHAR_TO_WCHAR(*CefLogFile);
	Settings.log_severity = LOGSEVERITY_VERBOSE; //bVerboseLogging ? LOGSEVERITY_VERBOSE : LOGSEVERITY_WARNING;
	...
	FString ResourcesPath(FPaths::Combine(*FPaths::ProjectPluginsDir(), CEF3_RESOURCES_DIR));
	ResourcesPath = FPaths::ConvertRelativePathToFull(ResourcesPath);
	if (!FPaths::DirectoryExists(ResourcesPath))
	{
		UE_LOG(LogYetiWebBrowser, Error, TEXT("Chromium Resources information not found at: %s."), *ResourcesPath);
	}
	...
	{
		UE_LOG(LogYetiWebBrowser, Error, TEXT("Chromium Locales information not found at: %s."), *LocalesPath);
	}
	...
	{
		UE_LOG(LogYetiWebBrowser, Error, TEXT("UnrealCEFSubProcess.exe not found, check that this program has been built and is placed in: %s."), *SubProcessPath);
	}
...
#if WITH_CEF3
void FYetiWebBrowserSingleton::HandleRenderProcessCreated(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefDictionaryValue> ExtraInfo)
{
...
FYetiWebBrowserSingleton::~FYetiWebBrowserSingleton()
{
#if WITH_CEF3
	{
		FScopeLock Lock(&WindowInterfacesCS);
	...
	// #TODO CEFBrowserApp->OnRenderProcessThreadCreated().Unbind();
...
TSharedRef<IYetiWebBrowserWindowFactory> FYetiWebBrowserSingleton::GetWebBrowserWindowFactory() const
{
	return WebBrowserWindowFactory;
}

TSharedPtr<IYetiWebBrowserWindow> FYetiWebBrowserSingleton::CreateBrowserWindow(
	TSharedPtr<FCEFWebBrowserWindow>& BrowserWindowParent,
...
TSharedPtr<IYetiWebBrowserWindow> FYetiWebBrowserSingleton::CreateBrowserWindow(const FCreateYetiBrowserWindowSettings& WindowSettings)
{
	...
	if (AllowCEF)
	{
		...
		{
			const FYetiBrowserContextSettings Context = WindowSettings.Context.GetValue();
		...
		CefRefPtr<CefBrowser> Browser = CefBrowserHost::CreateBrowserSync(WindowInfo, NewHandler.get(), TCHAR_TO_WCHAR(*WindowSettings.InitialURL), BrowserSettings, CefDictionaryValue::Create(), RequestContext);
		if (Browser.get())
		{
			// Create new window
...
#if BUILD_EMBEDDED_APP
TSharedPtr<IYetiWebBrowserWindow> FYetiWebBrowserSingleton::CreateNativeBrowserProxy()
{
...
bool FYetiWebBrowserSingleton::Tick(float DeltaTime)
{
	...
	for (int32 Index = WindowInterfaces.Num() - 1; Index >= 0; --Index)
	{
		...
		{
			TSharedPtr<IYetiWebBrowserWindow> BrowserWindow = WindowInterfaces[Index].Pin();
			if (BrowserWindow.IsValid())
			{
				// Test if we've ticked recently. If not assume the browser window has become hidden.
				BrowserWindow->CheckTickActivity();
...
FString FYetiWebBrowserSingleton::GetCurrentLocaleCode()
{
	FCultureRef Culture = FInternationalization::Get().GetCurrentCulture();
...
PRAGMA_DISABLE_DEPRECATION_WARNINGS
void FYetiWebBrowserSingleton::DeleteBrowserCookies(FString URL, FString CookieName, TFunction<void(int)> Completed)
{
...
TSharedPtr<IYetiWebBrowserCookieManager> FYetiWebBrowserSingleton::GetCookieManager(TOptional<FString> ContextId) const
{
	if (ContextId.IsSet())
	{
#if WITH_CEF3
		...
		if (ExistingContext)
		{
			...
			//return FCefWebBrowserCookieManagerFactory::Create((*ExistingContext));
		}
		else
		{
			UE_LOG(LogYetiWebBrowser, Log, TEXT("No cookie manager for ContextId=%s.  Using default cookie manager"), *ContextId.GetValue());
		}
#endif
	}
...
bool FYetiWebBrowserSingleton::RegisterContext(const FYetiBrowserContextSettings& Settings)
{
#if WITH_CEF3
...
bool FYetiWebBrowserSingleton::UnregisterContext(const FString& ContextId)
{
#if WITH_CEF3
...
bool FYetiWebBrowserSingleton::RegisterSchemeHandlerFactory(FString Scheme, FString Domain, IYetiWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
...
bool FYetiWebBrowserSingleton::UnregisterSchemeHandlerFactory(IYetiWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory)
{
#if WITH_CEF3
