...
public:
	...
	virtual void ConnectTo(const TSharedRef<IYetiWebBrowserWindow>& BrowserWindow) override
	{
		if (JSBridge != nullptr)
		{
	...
	virtual void DisconnectFrom(const TSharedRef<IYetiWebBrowserWindow>& BrowserWindow) override
	{
...
TSharedRef<IYetiWebBrowserAdapter> FWebBrowserAdapterFactory::Create(const FString& Name, UObject* JSBridge, bool IsPermanent)
{
	return MakeShareable(new FDefaultWebBrowserAdapter(Name, FString(), FString(), JSBridge, IsPermanent));
}

TSharedRef<IYetiWebBrowserAdapter> FWebBrowserAdapterFactory::Create(const FString& Name, UObject* JSBridge, bool IsPermanent, const FString& ConnectScriptText, const FString& DisconnectScriptText)
{
