...
FYetiWebJSParam::~FYetiWebJSParam()
{
...
FYetiWebJSParam::FYetiWebJSParam(const FYetiWebJSParam& Other)
	: Tag(Other.Tag)
{
	switch (Other.Tag)
	{
		...
		case PTYPE_STRUCT:
			...
			break;
		case PTYPE_ARRAY:
			ArrayValue = new TArray<FYetiWebJSParam>(*Other.ArrayValue);
			break;
		case PTYPE_MAP:
			MapValue = new TMap<FString, FYetiWebJSParam>(*Other.MapValue);
			break;
	}
}

FYetiWebJSParam::FYetiWebJSParam(FYetiWebJSParam&& Other)
	: Tag(Other.Tag)
{
	switch (Other.Tag)
	{
...
void FYetiWebJSCallbackBase::Invoke(int32 ArgCount, FYetiWebJSParam Arguments[], bool bIsError) const
{
	TSharedPtr<FYetiWebJSScripting> Scripting = ScriptingPtr.Pin();
