...
#include "CEF3/cef_binary_3.3071.1611.g4a19305_windows64/include/internal/cef_types.h"
...
FCEFWebBrowserWindow::FCEFWebBrowserWindow(CefRefPtr<CefBrowser> InBrowser, CefRefPtr<FCEFBrowserHandler> InHandler, FString InUrl, TOptional<FString> InContentsToLoad, bool bInShowErrorMessage, bool bInThumbMouseButtonNavigation, bool bInUseTransparency, bool bInJSBindingToLoweringEnabled)
	: DocumentState(EYetiWebBrowserDocumentState::NoDocument)
...
EYetiWebBrowserDocumentState FCEFWebBrowserWindow::GetDocumentLoadingState() const
{
...
bool FCEFWebBrowserWindow::OnJSDialog(CefJSDialogHandler::JSDialogType DialogType, const CefString& MessageText, const CefString& DefaultPromptText, CefRefPtr<CefJSDialogCallback> Callback, bool& OutSuppressMessage)
{
	...
	{
		TSharedPtr<IYetiWebBrowserDialog> Dialog(new FCEFWebBrowserDialog(DialogType, MessageText, DefaultPromptText, Callback));
		EYetiWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IYetiWebBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EYetiWebBrowserDialogEventResponse::Handled:
			Retval = true;
			break;
		case EYetiWebBrowserDialogEventResponse::Continue:
			...
			break;
		case EYetiWebBrowserDialogEventResponse::Ignore:
			OutSuppressMessage = true;
			Retval = false;
			break;
		case EYetiWebBrowserDialogEventResponse::Unhandled:
		default:
			Retval = false;
			break;
		}
...
bool FCEFWebBrowserWindow::OnBeforeUnloadDialog(const CefString& MessageText, bool IsReload, CefRefPtr<CefJSDialogCallback> Callback)
{
	...
	if ( OnShowDialog().IsBound() )
	{
		TSharedPtr<IYetiWebBrowserDialog> Dialog(new FCEFWebBrowserDialog(MessageText, IsReload, Callback));
		EYetiWebBrowserDialogEventResponse EventResponse = OnShowDialog().Execute(TWeakPtr<IYetiWebBrowserDialog>(Dialog));
		switch (EventResponse)
		{
		case EYetiWebBrowserDialogEventResponse::Handled:
...
void FCEFWebBrowserWindow::OnRenderProcessTerminated(CefRequestHandler::TerminationStatus Status)
{
	...
	{
		bRecoverFromRenderProcessCrash = false;
		NotifyDocumentError((int)-2); // Only attempt a single recovery at a time
	}
...
void FCEFWebBrowserWindow::NotifyDocumentError(int InErrorCode)
{
	...
	DocumentState = EYetiWebBrowserDocumentState::Error;
...
void FCEFWebBrowserWindow::NotifyDocumentLoadingStateChange(bool IsLoading)
{
	...
	if (IsLoading || DocumentState != EYetiWebBrowserDocumentState::Error)
	{
		ErrorCode = 0;
		DocumentState = IsLoading
			? EYetiWebBrowserDocumentState::Loading
			: EYetiWebBrowserDocumentState::Completed;
		DocumentStateChangedEvent.Broadcast(DocumentState);
	}
}
	...
	if (InternalCefBrowser != nullptr && InternalCefBrowser->IsSame(Browser))
	{
		...
		if (MainFrame.get() != nullptr)
		{
			if(OnBeforeBrowse().IsBound())
			{
				...
				FYetiWebNavigationRequest RequestDetails;
...
{
	bool bHandled = Scripting->OnProcessMessageReceived(Browser, Frame, SourceProcess, Message);
...
void FCEFWebBrowserWindow::OnBrowserClosed()
{
	...
	{
		OnCloseWindow().Execute(TWeakPtr<IYetiWebBrowserWindow>(SharedThis(this)));
	}
