...
bool FCEFJSScripting::OnProcessMessageReceived(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefProcessId SourceProcess, CefRefPtr<CefProcessMessage> Message)
{
	bool Result = false;
...
bool FCEFJSScripting::HandleExecuteUObjectMethodMessage(CefRefPtr<CefListValue> MessageArguments)
{
		...
		for ( TFieldIterator<FProperty> It(Function); It; ++It )
		{
			...
			if (Param->PropertyFlags & CPF_Parm)
			{
				...
				else
				{
					...
					if (StructProperty && StructProperty->Struct->IsChildOf(FYetiWebJSResponse::StaticStruct()))
					{
						PromiseParam = Param;
					}
					else
	...
	if (PromiseParam)
	{
		FYetiWebJSResponse* PromisePtr = PromiseParam->ContainerPtrToValuePtr<FYetiWebJSResponse>(Params);
		if (PromisePtr)
		{
			*PromisePtr = FYetiWebJSResponse(SharedThis(this), ResultCallbackId);
		}
	}
...
void FCEFJSScripting::InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FYetiWebJSParam Arguments[], bool bIsError)
{
	CefRefPtr<CefListValue> FunctionArguments = CefListValue::Create();
