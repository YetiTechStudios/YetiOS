...
void FCEFBrowserHandler::OnAfterCreated(CefRefPtr<CefBrowser> Browser)
{
	if(Browser->IsPopup())
	{
		...
		if(BrowserWindowParent.IsValid() && ParentHandler->OnCreateWindow().IsBound())
		{
			...
			TSharedPtr<IYetiWebBrowserWindow> NewBrowserWindow = IYetiWebBrowserModule::Get().GetSingleton()->CreateBrowserWindow(
				BrowserWindowParent,
			...
			bool bUIWindowCreated = ParentHandler->OnCreateWindow().Execute(TWeakPtr<IYetiWebBrowserWindow>(NewBrowserWindow), TWeakPtr<IYetiWebBrowserPopupFeatures>(BrowserPopupFeatures));
			if(!bUIWindowCreated)
			{
...
CefResourceRequestHandler::ReturnValue FCEFBrowserHandler::OnBeforeResourceLoad(CefRefPtr<CefBrowser> Browser, CefRefPtr<CefFrame> Frame, CefRefPtr<CefRequest> Request, CefRefPtr<CefRequestCallback> Callback)
{
	...
	CefPostTask(TID_UI, new FCEFBrowserClosureTask(this, [=]()
	{
		...
		const FString LocaleCode = FYetiWebBrowserSingleton::GetCurrentLocaleCode();
...
bool FCEFBrowserHandler::ShowDevTools(const CefRefPtr<CefBrowser>& Browser)
{
	...
	bool bSuppressWindowCreation = OnBeforePopup(Browser, Browser->GetFocusedFrame(), TargetUrl, TargetFrameName, PopupFeatures, WindowInfo, NewClient, BrowserSettings, ExtraInfo, &NoJavascriptAccess);
	...
	CefEventHandle OsEvent)
{
	...
	if( (Event.type == KEYEVENT_RAWKEYDOWN || Event.type == KEYEVENT_KEYDOWN) &&
		...
		IYetiWebBrowserModule::Get().GetSingleton()->IsDevToolsShortcutEnabled()
	  )
	{
