...
#if WITH_CEF3
#include "YetiWebJSFunction.h"
#include "YetiWebJSScripting.h"
...
public:
	...
	bool SetConverted(CefRefPtr<ContainerType> Container, KeyType Key, FYetiWebJSParam& Param)
	{
		switch (Param.Tag)
		{
			case FYetiWebJSParam::PTYPE_NULL:
				return Container->SetNull(Key);
			case FYetiWebJSParam::PTYPE_BOOL:
				return Container->SetBool(Key, Param.BoolValue);
			case FYetiWebJSParam::PTYPE_DOUBLE:
				return Container->SetDouble(Key, Param.DoubleValue);
			case FYetiWebJSParam::PTYPE_INT:
				return Container->SetInt(Key, Param.IntValue);
			case FYetiWebJSParam::PTYPE_STRING:
			{
				CefString ConvertedString = TCHAR_TO_WCHAR(**Param.StringValue);
				return Container->SetString(Key, ConvertedString);
			}
			case FYetiWebJSParam::PTYPE_OBJECT:
			{
			...
			case FYetiWebJSParam::PTYPE_STRUCT:
			{
			...
			case FYetiWebJSParam::PTYPE_ARRAY:
			{
			...
			case FYetiWebJSParam::PTYPE_MAP:
			{
	...
	void InvokeJSFunction(FGuid FunctionId, int32 ArgCount, FYetiWebJSParam Arguments[], bool bIsError=false) override;
