...
class IYetiWebBrowserSchemeHandlerFactory;
class UMaterialInterface;
struct FWebBrowserWindowInfo;

class IYetiWebBrowserWindowFactory
{
public:
...
struct YETIWEBBROWSER_API FYetiBrowserContextSettings
{
	FYetiBrowserContextSettings(const FString& InId)
		: Id(InId)
		, AcceptLanguageList()
...
struct YETIWEBBROWSER_API FCreateYetiBrowserWindowSettings
{

	FCreateYetiBrowserWindowSettings()
	...
	TOptional<FYetiBrowserContextSettings> Context;
 ...
 */
class YETIWEBBROWSER_API IYetiWebBrowserSingleton
{
public:
	/**
	 * Virtual Destructor
	 */
	virtual ~IYetiWebBrowserSingleton() {};

	/** @return A factory object that can be used to construct additional WebBrowserWindows on demand. */
	virtual TSharedRef<IYetiWebBrowserWindowFactory> GetWebBrowserWindowFactory() const = 0;
	 ...
	 */
	virtual TSharedPtr<IYetiWebBrowserWindow> CreateBrowserWindow(
		TSharedPtr<FCEFWebBrowserWindow>& BrowserWindowParent,
	...
	virtual TSharedPtr<IYetiWebBrowserWindow> CreateBrowserWindow(const FCreateYetiBrowserWindowSettings& Settings) = 0;

#if	BUILD_EMBEDDED_APP
	virtual TSharedPtr<IYetiWebBrowserWindow> CreateNativeBrowserProxy() = 0;
#endif
	...
	virtual TSharedPtr<class IYetiWebBrowserCookieManager> GetCookieManager() const = 0;

	virtual TSharedPtr<class IYetiWebBrowserCookieManager> GetCookieManager(TOptional<FString> ContextId) const = 0;

	virtual bool RegisterContext(const FYetiBrowserContextSettings& Settings) = 0;
	 ...
	 */
	virtual bool RegisterSchemeHandlerFactory(FString Scheme, FString Domain, IYetiWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory) = 0;
	 ...
	 */
	virtual bool UnregisterSchemeHandlerFactory(IYetiWebBrowserSchemeHandlerFactory* WebBrowserSchemeHandlerFactory) = 0;
